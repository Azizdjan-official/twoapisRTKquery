import React, { useState } from "react";
import { useForm } from "react-hook-form";
import { useGetUsersQuery, usePostUsersMutation } from "../redux/service/user-api";
import { Usercard } from './../components/Usercard';


export const UserPage = () => {
  const { register, reset, handleSubmit } = useForm();
  const [postUser] = usePostUsersMutation();
  const [page, setPage] = useState(1);
  const { data, isLoading } = useGetUsersQuery(page);
  
  const submit = (data) => {
    postUser(data)
    .unwrap()
    .then((res) => {
      console.log(res);
    });
    reset();
  };

  const changePAge = (id) => {
    setPage(id);
  }
  const buttons = Array(data?.pageSize).fill(null);
  return <div>
        <h1 className="text-3xl text-center font-bold">User Lists</h1>
      <div className="flex flex-col justify-center items-center my-3 ">
        <form className="w-[60%] p-5 shadow-lg border border-white rounded-lg bg-slate-600"  onSubmit={handleSubmit(submit)}>
          <div className="mb-3">
            <input
              className="py-2 w-full border px-2 border-blue-500"
              {...register("name", { required: true })}
              placeholder="name"
              type="text"
            />
          </div>
          <div className="mb-3">
            <input
              className="py-2 w-full border px-2 border-blue-500"
              {...register("email", { required: true })}
              placeholder="email"
              type="text"
            />
          </div>
          <div className="flex  justify-center">
            <button className="bg-blue-500 text-white rounded-md font-bold p-2 w-[50%]" type="submit">
              send
            </button>
          </div>
        </form>
      </div>
      <div className="h-[49vh] my-4">
      {isLoading ? (
        <h1 className="text-4xl">Loading...</h1>
      ) : (
        <>
          {data?.data?.map((item) => (
            <Usercard key={item.id} {...item} />
          ))}
        </>
      )}
      </div>
      <div className="flex items-center justify-center  gap-3">
        {buttons.map((_, index) => {
          let number = index + 1;
          return (
            <button
              onClick={() => changePAge(number)}
              className={`p-3 bg-red-400 ${
                number === page ? "bg-blue-400" : ""
              }`}
              key={number}
            >
              {number}
            </button>
          );
        })}
      </div>
  </div>;
};
